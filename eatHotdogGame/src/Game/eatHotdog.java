...
//继承窗体类
public class eatHotdog extends Frame{

    public static int width = 900, height = 900; //窗口大小

    Image background = GameUtil.getImage("images/bg.jpg"); //背景图片
    Plane plane = new Plane("images/WSC.png", 200, 200);
	...
	class MusicPlayer implements Runnable{
		...
		public void run() {
			try {
				new Player(new FileInputStream(eatHotdog.class.getClassLoader().getResource("music/src_raw_music.mp3").getPath().substring(1))).play();
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
